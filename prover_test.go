...
func TestProver_GenerateProveNativeCalldata(t *testing.T) {
	// Create test data
	...
	expectedSelector := hexutil.Encode(nativeProverABI.Methods["proveNative"].ID)
	...
	err = nativeProverABI.Methods["proveNative"].Inputs.UnpackIntoMap(unpackedMap, hexCalldata[4:])
